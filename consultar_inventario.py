...
def quick_intent(user_text: str) -> dict | None:
    t = user_text.strip().lower()
    
    # Detectar intención de LISTAR/VER inventario
    if any(word in t for word in ["listar", "ver", "mostrar", "inventario", "lista", "muestra", "productos", "qué hay", "que hay", "disponible"]):
        return {"action": "listar", "name": None, "quantity": None, "description": None, "id": None}
    
    # Detectar intención de AÑADIR (frases más naturales)
    if any(phrase in t for phrase in [
        "añadir", "agregar", "añada", "agregue", "agrega", "añade", "crear", "nuevo", 
        "quiero añadir", "quiero agregar", "voy a añadir", "voy a agregar",
        "quiero crear", "necesito añadir", "necesito agregar", "registrar"
    ]):
        return {"action": "añadir", "name": None, "quantity": None, "description": None, "id": None}
    
    # Detectar intención de ACTUALIZAR/MODIFICAR (frases más naturales)
    if any(phrase in t for phrase in [
        "actualizar", "cambiar", "modificar", "editar", "update", "cambio",
        "quiero cambiar", "quiero modificar", "quiero actualizar", "quiero editar",
        "voy a cambiar", "voy a modificar", "necesito cambiar", "necesito modificar"
    ]):
        return {"action": "actualizar", "name": None, "quantity": None, "description": None, "id": None}
    
    # Detectar intención de ELIMINAR (frases más naturales)
    if any(phrase in t for phrase in [
        "eliminar", "borrar", "quitar", "delete", "remover", "elimina", "borra",
        "quiero eliminar", "quiero borrar", "quiero quitar", 
        "voy a eliminar", "voy a borrar", "necesito eliminar", "necesito borrar"
    ]):
        return {"action": "eliminar", "name": None, "quantity": None, "description": None, "id": None}
    
    # Detectar MENÚ
    if any(word in t for word in ["menu", "menú", "inicio", "home", "volver", "regresar", "atrás"]):
        return {"action": "menu", "name": None, "quantity": None, "description": None, "id": None}
    
    # Detectar SALIR
    if any(word in t for word in ["salir", "exit", "quit", "cerrar", "terminar", "adios", "adiós", "chao", "bye"]):
...
def main() -> None:
    ...
    while True:
        ...
        try:
            intent = quick_intent(user_text) or call_llm(user_text)
        except Exception as e:
